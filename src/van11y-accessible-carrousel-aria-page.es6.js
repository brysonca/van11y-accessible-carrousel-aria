...
(doc => {
    ...
    const createControlListItem = config => {
        ...
        return `<li class="${CARROUSEL_CONTROL_LIST_ITEM} ${itemClass}" ${ATTR_ROLE}="${ATTR_PRESENTATION}"><a class="${CARROUSEL_CONTROL_LIST_LINK} ${linkClass}" id="${id}" ${ATTR_ROLE}="${ATTR_TAB}" ${ATTR_CONTROL}="${controlId}" ${ATTR_SELECTED}="${isSelected}" ${CARROUSEL_DATA_ELEMENT_NUMBER}="${numberElement}" tabindex="${tabindexValue}">${numberElement}</a></li>`; //CWI        
	};
